...
def sum(f: Int => Int): (Int, Int) => Int = sumf
...
val f3: Int => Int => Int = f2.curried
// transform it to a curried version (type is Int => Int => Int)
val f4: (Int, Int) => Int = Function.uncurried(f3)
// go back to the uncurried version (type is (Int, Int) => Int)
...
class MyClass(                // Defines a new type MyClass with a constructor
               x: Int,        // x will not be available outside MyClass
               val y: Int,    // val will generate a getter for y
               var z: Int) {  // var will generate a getter and a setter for z
  require(y > 0, "y must be positive")
  // precondition, triggering an IllegalArgumentException if not met
  def this(x: Int) = this(x, 2, 1)
...
val mc: MyClass = new MyClass(3)
val mcY = mc.y.toString
val mcZ = mc.z.toString
// mc.y = 123
// it's not possible to define another value to val, just to var
mc.z = 123
...
class Level1 extends TopLevel { // extend TopLevel class
  def method1(x: Int): Int = x  // implement abstract method
...
All members of packages scala and java.lang as well as
all members of the object scala.Predef are automatically imported.
...
// example using extends and trait
...
new MyClass[Int](1)
new MyClass(1)
// the type is being inferred, i.e. determined based on the value arguments
